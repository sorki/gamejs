...
/**
 * Dirty buffers tracking, subclass of `Group` class
 * @constructor
 */
var RenderUpdates = exports.RenderUpdates = function() {
   RenderUpdates.superConstructor.apply(this, arguments);
   return this;
}
objects.extend(RenderUpdates, Group);

/*
 * Add one or more sprites to this group. Also creates
 * `sprite._old_rect` property to track updates of the sprite.
 * @param {Array|gamejs.sprite.Sprite} sprites One or more
 * `gamejs.sprite.Sprite` instances
 */
RenderUpdates.prototype.add = function(sprites) {
   if (!(sprites instanceof Array)) sprites = [sprites];

   sprites.forEach(function(sprite) {
      this._sprites.push(sprite);
      sprite._groups.push(this);
      sprite._old_rect = sprite.rect;
   }, this);
   return;
};
 ...
 * Draw all the sprites in this group. This is equivalent to calling each
 * sprite's draw method.
 * @returns {Array} of `gamejs.Rect` - rectangles changed by this draw
 * containing both old position and current position
 */
RenderUpdates.prototype.draw = function() {
   var args = arguments;
   var dirty = [];
   var srect, nrect;
   this._sprites.forEach(function(sprite) {
      srect = sprite._old_rect;
      nrect = sprite.draw.apply(sprite, args);
      if (srect.collideRect(nrect)) {
         dirty.push(srect.union(nrect));
      } else {
         dirty.push(srect);
         dirty.push(nrect);
      }
      sprite._old_rect = nrect;
   }, this);
   return dirty;
}
